...
- **2026-05-19 (Executor)**: **14-1 사업자 진위확인 제외** — 고도화 범위에서 제외하기로 한 기능이 마이페이지에 남아 있어 `BusinessVerifyCard`·`/api/profile/verify-business`·NTS 클라이언트 등 제거. 프로필은 수동 입력만 유지. — 프로필 온보딩·스포트라이트 API, 검색 0건 조건 칩 해제, 진단→프로필 저장, 자격 대안 공고, 검색 카드 찜·중복 배지, SEO(`/programs/closing-soon`, `/programs/region/*`, sitemap), Resend 알림 HTML·수신설정 링크, `search_text` 마이그레이션 파일·백필 스크립트. `npm run build` PASS. **미커밋**. **블로커**: 원격 DB에 `search_text` 컬럼 미적용 — Supabase Dashboard SQL Editor에서 `supabase/migrations/20260519120000_support_programs_search_text.sql` 실행 후 `SUPPORT_PROGRAMS_SEARCH_TEXT=1` env 설정·`npx tsx scripts/backfill-program-search-fields.ts` 권장. `supabase login` 없어 CLI `db push` 불가. — (1) 알림·관리자 필터 출처 `smba`→`smes24` 통일 (`lib/gov-support/programSources.ts`, 레거시 `smba` 매칭 호환). (2) `/admin` → `/admin/dashboard` 리다이렉트 페이지. (3) `verify:journey-exhaustive` 관리자·알림 섹션(+10), `verify:admin` sync/verify·heal·bizinfo-verify 차단 검증. `verify:journey-exhaustive` 58/58·`verify:admin`·`npm run build` PASS. **미커밋** — 커밋·푸시·프로덕션 스모크(관리자 로그인·공고 필터 중소벤처24) 요청.
